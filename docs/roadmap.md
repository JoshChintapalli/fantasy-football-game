...
- [ ] Player Selection Per Posistion - Give options of best players, given the game initialization and team spin, for each open position on the roster and allow only one selection per spin
- [ ] Roster Management - Track all filled and open positions in the roster to only allow selections to go in allowed positions
- [ ] Score Calculation - Track the points associated with each player selection and add all of them together at the end of the round to calculate the final roster score
- [ ] Basic UI in Streamlit - Build functioning UI that allows users to see their roster, spin the team wheel, and make a player selection
# Future Features
...
- [ ] Bonus Round - Have a round after all roster positions are filled where a new random week, season, and team are selected, showing the best available player for each position, allowing the replacement of a player on the roster with a higher scoring one
...
# Technical Debt
- [ ] Empty data points and NaN Values - Some data points for a player do not have recorded data that could break game logic and points calculation
- [ ] Invalid Player Assignment - Selected players can be placed anywhere on the roster but should only be placed in their assignment position or other valid positions
